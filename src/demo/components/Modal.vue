...
<script>
  ...
  export default {
    ...
    mounted () {
      window.$modal.fromComponent(DefaultModal, {
        title: '模态窗标题',
        theme: 'default',
        onHide: () => {
      ...
      window.$modal.fromComponent(MultiModal, {
        title: '模态窗标题',
        theme: 'dark'
      }).then((modal) => {
      ...
      window.$modal.fromComponent(PopupModal, {
        title: '模态窗内弹层',
        theme: 'energized'
      }).then((modal) => {
    ...
    destroyed () {
      if (this.modal) {
        window.$modal.destroy(this.modal)
      }

      if (this.multiModal) {
        window.$modal.destroy(this.multiModal)
      }

      if (this.popupModal) {
        window.$modal.destroy(this.popupModal)
      }
