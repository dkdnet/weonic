<template>
  <div class="page has-navbar" v-nav="{title: '下拉刷新、无限加载', showBackButton: true}">
    <scroll class="page-content"
      ...
      <item v-for="item in items" :key="item.index" @click.native="onItemClick(item.index)"
            :class="{'item-stable': item.index % 2 == 0}">
        {{ item.content }}
      </item>
...
<script>
  export default {
    ...
    mounted () {
      for (let i = 1; i <= 20; i++) {
        this.items.push({'index': i, 'content': i + ' - keep walking, be 2 with you.'})
      }
      this.top = 1
      this.bottom = 20
    },
