...
<script>
  export default {
    ...
    mounted () {
      ...
      `
      this.sidebar = window.$sidebar.fromTemplate(template, {position: 'left'})

      this.sidebarRight = window.$sidebar.fromTemplate('<h5>右边栏</h5>', {position: 'right'})

      // setTimeout(() => {window.$app.setTitle('标题文字')}, 1000)
    },
    ...
    methods: {
      back () {
        window.$router.back({path: '/'})
      },

      loading (tips) {
        window.$loading.show(tips)
      ...
      toast () {
        window.$toast.show('发送成功', 1500).then(() => {
